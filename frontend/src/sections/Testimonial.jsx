...
export default function Testimonial() {
  ...
  const [step, setStep] = useState(testimonialsData.length);
  ...
  ? Math.min(screenWidth - 10, 360) // 🔼 plus large mobile
  : isTablet
  ? Math.min((screenWidth - 60) / 2, 360)
  : 360;
  const stepSize = cardWidth + gap;

  // Triple copie pour boucle infinie fluide
  const loopData = [...testimonialsData, ...testimonialsData, ...testimonialsData];

  // Même rythme tout le temps
  useEffect(() => {
    const interval = setInterval(() => {
      setStep((prev) => prev + 1);
    }, 4000); // même rythme constant
...
useEffect(() => {
  ...
  if (step >= len * 2) {
    const timeout = setTimeout(() => {
      ...
      // attendre 2 frames avant de remettre transition
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          setNoTransition(false);
        });
      });
    }, 1500); // même valeur que transition CSS
  ...
  const activeDot =
    ((step - testimonialsData.length) % testimonialsData.length + testimonialsData.length) %
...
return (
 ...
 <h2 className="testimonialsTitle">
  Approuvé par <br />
  <span>nos</span> <span>lecteurs</span>
</h2>

 <div className="testimonialsWrapper">

<button className="tsArrow left" onClick={prevSlide}>
  <ArrowIcon className="arrowSvg leftArrow" />
</button>

  <div className="testimonialsViewport">
    <div
      className={`testimonialsTrack ${noTransition ? "noTransition" : ""}`}
      style={{
        transform: `translateX(-${step * stepSize}px)`,
        gap: `${gap}px`,
      }}
    >
      {loopData.map((item, i) => (
        ...
        >
         ...
         <img src={item.logo} className="tsLogo" alt={`${item.name} logo`} />
            ...
            <div className="tsHeadText">
              ...
              <p className="tsRole">{item.role}</p>
            </div>
          </div>

          <div className="tsBody">
            <p className="tsText">{item.text}</p>
          </div>
 ...
 <button className="tsArrow right" onClick={nextSlide}>
  <ArrowIcon className="arrowSvg" />
</button>
    ...
    <div className="tsDots">
      {testimonialsData.map((_, index) => (
        <button
          key={index}
          className={`tsDot ${activeDot === index ? "active" : ""}`}
          onClick={() => {
            setNoTransition(false);
            setStep(testimonialsData.length + index);
          }}
